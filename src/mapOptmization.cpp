...
mapOptimization::mapOptimization() {
  ...
  pub_key_info_         = nh.advertise<lio_sam::KeyPosesAndFrames>("lio_sam/feature/key_info", 1);
...
void mapOptimization::pointAssociateToMap(PointType const *const pi, PointType *const po) {
  po->x = transPointAssociateToMap(0, 0) * pi->x + transPointAssociateToMap(0, 1) * pi->y + transPointAssociateToMap(0,
      2) * pi->z + transPointAssociateToMap(0, 3);
  po->y = transPointAssociateToMap(1, 0) * pi->x + transPointAssociateToMap(1, 1) * pi->y + transPointAssociateToMap(1,
      2) * pi->z + transPointAssociateToMap(1, 3);
  po->z = transPointAssociateToMap(2, 0) * pi->x + transPointAssociateToMap(2, 1) * pi->y + transPointAssociateToMap(2,
...
void mapOptimization::addOdomFactor() {
  if (cloudKeyPoses3D->points.empty()) {
    noiseModel::Diagonal::shared_ptr priorNoise = noiseModel::Diagonal::Variances((Vector(6) << 1e-2, 1e-2, M_PI * M_PI, 1e8, 1e8,
...
void mapOptimization::saveKeyFramesAndFactor() {
  ...
  publishKeyPoseAndSurfCloudKeyFrames();
}

void mapOptimization::publishKeyPoseAndSurfCloudKeyFrames() {
  lio_sam::KeyPosesAndFrames key_poses_and_frames;
  MYLOG_INFO << "key pose size: " << cloudKeyPoses6D->size() << ", key surf frame size: " << surfCloudKeyFrames.size();
  for (size_t i = 0; i < cloudKeyPoses3D->size(); ++i) {
    ...
    pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(cloudKeyPoses6D->points[i].roll,
      cloudKeyPoses6D->points[i].pitch, cloudKeyPoses6D->points[i].yaw);
    key_poses_and_frames.key_poses.push_back(pose);
    sensor_msgs::PointCloud2 pointcloud;
    pcl::toROSMsg(*surfCloudKeyFrames[i], pointcloud);
    key_poses_and_frames.key_frame_clouds.push_back(pointcloud);
  }
  key_poses_and_frames.header.frame_id = odometryFrame;
  pub_key_info_.publish(key_poses_and_frames);
}
...
void mapOptimization::publishOdometry() {
  ...
  laserOdometryROS.pose.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(transformTobeMapped[0], transformTobeMapped[1],
...
int main(int argc, char **argv) {
  ...
  MYLog mylog("lio_sam_map_optimization");
  mapOptimization MO;
